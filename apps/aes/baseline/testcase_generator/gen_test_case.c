...
/*
    ...
    bit_num = 256;
    word ctr_key[8];
    rand_word_array(ctr_key, bit_num);
    printf("AES-%d test cases:\n\n", bit_num);
    for(i=0; i<num_case; i++) {
        rand_word_array(state, 128);
        // rand_word_array(key, bit_num);
        ctr_key[7]++;
        if (ctr_key[7] == 0) {  // overflow
            printf("Overflow!\n");
            return 1;
        }
        printf("plaintext: ");
        ...
        printf("\n");
        printf("key:       ");
        print_verilog_hex(ctr_key, bit_num);
        printf("\n");
        encrypt_256_key_expand_inline_no_branch(state, ctr_key);
        printf("ciphertext:");
        print_verilog_hex(state, 128);
        printf("\n\n");       
    }
